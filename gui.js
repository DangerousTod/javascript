...
function MakeUserMove() {
	if (UserMove.from != SQUARES.NO_SQ && UserMove.to != SQUARES.NO_SQ) {
		...
		console.log("UserMove: " + PrSq(UserMove.from) + PrSq(UserMove.to));
...
function DrawMaterial() {
	...
	if(GameBoard.pceNum[PIECES.wQ]!=0 || GameBoard.pceNum[PIECES.bR]!=0 ||
...
function CheckResult() {
	...
	for ( MoveNum = GameBoard.moveListStart[GameBoard.ply]; MoveNum < GameBoard.moveListStart[GameBoard.ply + 1]; ++MoveNum) {
		if ( MakeMove(GameBoard.moveListStart[MoveNum]) == BOOL.FALSE) {
			continue;
		}
		found++;
		TakeMove();
	...
	if(InCheck == BOOL.TRUE) {
		if(GameBoard.side == COLOURS.WHITE) {
			$('#GameStatus').text("GAME OVER CHECKMATE BY BLACK");
			return BOOL.TRUE;
		} else {
			$('#GameStatus').text("GAME OVER CHECKMATE BY WHITE");return BOOL.TRUE;
		}
	} else {
		$('#GameStatus').text("STALEMATE GAME OVER");return BOOL.TRUE;
	}
	return BOOL.FALSE;
